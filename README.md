...
- Resume download from `public/assets/VikrantDey.pdf`
- Contact form with optional Resend email integration
- Top-right guestbook widget
  - Realtime mode when `NEXT_PUBLIC_WS_URL` is configured
  - Vercel-friendly fallback mode when no websocket backend is provided
...
- `NEXT_PUBLIC_WS_URL` is only needed if you want the guestbook to work in realtime.
- Without `NEXT_PUBLIC_WS_URL`, the guestbook shows a safe fallback panel instead of broken realtime UI.

## Project Structure

```text
src/app                 App Router pages and API routes
src/components          UI, layout, motion, and guestbook components
...
- The guestbook does not need a websocket backend to deploy cleanly on Vercel.
- If you later want realtime guestbook behavior, host a websocket server separately and set `NEXT_PUBLIC_WS_URL`.

## Content Notes

- Resume file path: `public/assets/VikrantDey.pdf`
